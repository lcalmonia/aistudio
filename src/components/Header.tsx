        ...
        <div className="flex items-center gap-2">
          {logoUrl ? (
            <img
              src={logoUrl}
              alt="Logo"
              className="w-7 h-7 rounded-lg object-cover border border-[#dec1af]"
          ...
          <h1 className="font-serif text-[17px] sm:text-[19px] font-bold text-[#26170c] tracking-tight truncate max-w-[150px] sm:max-w-[240px]">
            {storeName}
          </h1>
      ...
      <div className="flex items-center gap-2 sm:gap-2.5">
        {onSwitchToCustomerPortal && (
          <button
            ...
            className="px-2.5 sm:px-3 py-1.5 bg-[#f3ecea] hover:bg-[#e1e1c9] text-[#26170c] text-xs font-bold rounded-xl border border-[#dec1af]/60 transition-all flex items-center gap-1.5 cursor-pointer shadow-xs"
            title="Open Customer Order Storefront"
          >
        ...
        <button
          ...
          aria-label="Open Cart and POS"
        >
          <span className="material-symbols-outlined text-[24px]">shopping_cart</span>
          {activeOrdersCount > 0 && (
            <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] flex items-center justify-center bg-[#26170c] text-white text-[10px] font-bold rounded-full px-1">
              {activeOrdersCount}
            </span>
          )}
        </button>
