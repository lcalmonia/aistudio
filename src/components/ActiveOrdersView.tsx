...
export const ActiveOrdersView: React.FC<ActiveOrdersViewProps> = ({ orders = [], onUpdateOrderStatus, onOpenNewOrder, onShowNotification, onRefreshOrders, isSyncing = false, admin }) => {
  ...
  const [splitLines, setSplitLines] = useState<Array<{ method: 'GCash' | 'Maya' | 'Cash' | 'Card'; amount: number }>>([{ method: 'Cash', amount: 0 }, { method: 'GCash', amount: 0 }]);
  ...
  const allCount = orderList.filter(Boolean).length;
  ...
  const openPayment = (order: Order) => { setPaymentOrder(order); setPaymentMode('single'); setSinglePayment((order.paymentMethod as 'GCash' | 'Maya' | 'Cash' | 'Card') || 'Cash'); setSplitLines([{ method: 'Cash', amount: 0 }, { method: 'GCash', amount: 0 }]); setPaymentError(''); };

  const handleStatusAdvance = (order: Order) => {
    if (order.status === 'New') { onUpdateOrderStatus(order.id, 'Brewing'); onShowNotification(`Order #${order.orderNumber} is now brewing! ☕`); }
    else if (order.status === 'Brewing' || order.status === 'Preparing') { onUpdateOrderStatus(order.id, 'Ready'); onShowNotification(`Order #${order.orderNumber} marked ready for pickup! 🔔`); }
    else if (order.status === 'Ready') openPayment(order);
  };

  const handleRemindCustomer = (order: Order) => { onShowNotification(`Sent SMS reminder to ${order.customerName} (${order.customerPhone || 'customer'})! 📱`); };

  const handleResetOrders = async () => {
    if (!isSuperAdmin) return;
    if (!window.confirm('RESET ALL ORDERS? This permanently deletes every current and previous transaction from the Orders tab. This does not change menu, inventory, customers, or rewards.')) return;
    try { const { orderResetService } = await import('../services/orderResetService'); const deletedOrders = await orderResetService.resetAllOrders(); setSelectedFilter('All'); if (onRefreshOrders) await onRefreshOrders(); onShowNotification(`Orders reset successfully. ${deletedOrders} transaction(s) deleted.`); }
    catch (error) { console.error('[ActiveOrdersView] Failed to reset orders:', error); onShowNotification('Unable to reset orders. No records were deleted.'); }
  };

  const handleCancelTransaction = async (order: Order) => {
    if (!isSuperAdmin || orderActionId) return;
    if (!window.confirm(`Cancel transaction #${order.orderNumber}? The transaction will remain in the Orders tab as CANCELLED.`)) return;
    setOrderActionId(order.id);
    try { await orderService.cancelOrder(order.id); if (onRefreshOrders) await onRefreshOrders(); onShowNotification(`Transaction #${order.orderNumber} cancelled.`); }
    catch (error) { console.error('[ActiveOrdersView] Failed to cancel order:', error); onShowNotification(error instanceof Error ? error.message : 'Unable to cancel transaction.'); }
    finally { setOrderActionId(null); }
  };

  const handleDeleteTransaction = async (order: Order) => {
    if (!isSuperAdmin || orderActionId) return;
    if (!window.confirm(`DELETE transaction #${order.orderNumber}? This permanently removes the transaction and its order items from the database. This action cannot be undone.`)) return;
    setOrderActionId(order.id);
    try { await orderService.deleteOrder(order.id); if (onRefreshOrders) await onRefreshOrders(); onShowNotification(`Transaction #${order.orderNumber} deleted.`); }
    catch (error) { console.error('[ActiveOrdersView] Failed to delete order:', error); onShowNotification(error instanceof Error ? error.message : 'Unable to delete transaction.'); }
    finally { setOrderActionId(null); }
  };

  const splitTotal = splitLines.reduce((sum, line) => sum + Math.max(0, Number(line.amount) || 0), 0);
  const orderTotal = paymentOrder?.total || 0; const splitMatches = Math.abs(splitTotal - orderTotal) < 0.01;

  const completeOrder = async () => {
    if (!paymentOrder) return;
    if (paymentMode === 'single') { const order = paymentOrder; setPaymentOrder(null); onUpdateOrderStatus(order.id, 'Completed', singlePayment); confetti({ particleCount: 40, spread: 60, origin: { y: 0.8 }, colors: ['#26170c', '#8fbc8f', '#e1e1c9', '#fbddca'] }); onShowNotification(`Order #${order.orderNumber} completed via ${singlePayment}. Have a great day.`); return; }
    const validLines = splitLines.filter((line) => Number(line.amount) > 0);
    if (validLines.length < 2) { setPaymentError('Please enter amounts for at least two payment methods.'); return; }
    if (!splitMatches) { setPaymentError(`Payment total must equal ${money(orderTotal)}. Current total is ${money(splitTotal)}.`); return; }
    const order = paymentOrder; const breakdown = validLines.map((line) => `${line.method} ${money(line.amount)}`).join(' + '); setPaymentOrder(null);
    onUpdateOrderStatus(order.id, 'Completed', (`Split Payment: ${breakdown}` as unknown) as 'GCash' | 'Maya' | 'Cash' | 'Card');
    confetti({ particleCount: 40, spread: 60, origin: { y: 0.8 }, colors: ['#26170c', '#8fbc8f', '#e1e1c9', '#fbddca'] }); onShowNotification(`Order #${order.orderNumber} completed with split payment: ${breakdown}.`);
  };
    ...
    <div className="pt-20 pb-28 px-3.5 sm:px-5 w-full max-w-[1400px] mx-auto">
      <section className="mb-4">
        ...
        <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrolling-hide no-scrollbar">
          {(['All', 'Today', 'New', 'Brewing', 'Ready', ...(completedCount > 0 ? ['Completed'] : [])] as const).map((filter) => {
            const label = filter === 'All' ? `All (${allCount})` : filter === 'Today' ? `Today (${todayCount})` : filter === 'New' ? `New (${newCount})` : filter === 'Brewing' ? `Brewing (${brewingCount})` : filter === 'Ready' ? `Ready (${readyCount})` : `Completed (${completedCount})`;
      ...
      <div className="space-y-4">
        {filteredOrders.length === 0 ? (
          ...
          const isNew = order.status === 'New'; const isBrewing = order.status === 'Brewing' || order.status === 'Preparing'; const isReady = order.status === 'Ready'; const isCompleted = order.status === 'Completed'; const isCancelled = order.status === 'Cancelled';
          ...
          else if (isCancelled) { borderClass = 'border-l-4 border-[#a52b22]'; badgeClass = 'bg-[#ffdad6] text-[#93000a]'; statusLabel = 'CANCELLED'; }
