...
type PaymentMethod = 'GCash' | 'Maya' | 'Cash' | 'Card';
type SplitLine = { method: PaymentMethod; amount: number };

interface ActiveOrdersViewProps {
  orders: Order[];
  onUpdateOrderStatus: (orderId: string, newStatus: OrderStatus, paymentMethod?: PaymentMethod) => void;
  onOpenNewOrder: () => void;
...
const PAYMENT_METHODS: PaymentMethod[] = ['GCash', 'Maya', 'Cash', 'Card'];
...
export const ActiveOrdersView: React.FC<ActiveOrdersViewProps> = ({
  orders = [], onUpdateOrderStatus, onOpenNewOrder, onShowNotification, onRefreshOrders, isSyncing = false, admin,
}) => {
  ...
  const [singlePayment, setSinglePayment] = useState<PaymentMethod>('Cash');
  const [splitLines, setSplitLines] = useState<SplitLine[]>([{ method: 'Cash', amount: 0 }, { method: 'GCash', amount: 0 }]);
  ...
  const openPayment = (order: Order) => {
    ...
    setSinglePayment((order.paymentMethod as PaymentMethod) || 'Cash');
    setSplitLines([{ method: 'Cash', amount: 0 }, { method: 'GCash', amount: 0 }]);
    setPaymentError('');
  };

  const splitTotal = splitLines.reduce((sum, line) => sum + (Number(line.amount) || 0), 0);
  ...
  const completeWithPayment = () => {
    ...
    if (!splitMatches) {
      setPaymentError(`Payment amounts must equal ${money(orderTotal)}. Current total: ${money(splitTotal)}.`);
      return;
    }
    const validLines = splitLines.filter(line => Number(line.amount) > 0);
    if (validLines.length < 2) {
      setPaymentError('Split Payment requires at least two payment methods with an amount.');
      return;
    }
    const order = paymentOrder;
    const breakdown = validLines.map(line => `${line.method} ${money(line.amount)}`).join(' + ');
    setPaymentOrder(null);
    onUpdateOrderStatus(order.id, 'Completed', (`Split Payment: ${breakdown}` as unknown) as PaymentMethod);
    confetti({ particleCount: 40, spread: 60, origin: { y: 0.8 }, colors: ['#26170c', '#8fbc8f', '#e1e1c9', '#fbddca'] });
  ...
  const filters: Array<[typeof selectedFilter, string]> = [
    ['All', `All (${allActiveCount})`], ['Today', `Today (${todayCount})`], ['New', `New (${newCount})`],
    ['Brewing', `Brewing (${brewingCount})`], ['Ready', `Ready (${readyCount})`],
  ];
  if (completedCount > 0) filters.push(['Completed', `Completed (${completedCount})`]);
      ...
      <section className="mb-4">
        <div className="flex justify-between items-center mb-1">
          ...
          <div className="flex items-center gap-2">
            {onRefreshOrders && <button type="button" onClick={onRefreshOrders} disabled={isSyncing} title="Sync orders from server" className="flex items-center justify-center p-1.5 rounded-full text-[#636451] hover:bg-[#e1e1c9] transition-all disabled:opacity-50"><span className={`material-symbols-outlined text-[18px] ${isSyncing ? 'animate-spin' : ''}`}>sync</span></button>}
            {isSuperAdmin && <button type="button" onClick={handleResetOrders} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-bold bg-[#ffdad6] text-[#93000a] border border-[#f2b8b5] hover:bg-[#ffc9c4]"><span className="material-symbols-outlined text-[15px]">delete_sweep</span>Reset Orders</button>}
            <span className="inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-semibold px-2.5 py-1 bg-[#e1e1c9] text-[#636451] rounded-full"><span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />Live Server</span>
          </div>
        </div>
        <div className="flex gap-2 mt-4 overflow-x-auto pb-2 no-scrollbar">
          {filters.map(([filter, label]) => <button key={filter} onClick={() => setSelectedFilter(filter)} className={`px-3.5 sm:px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap flex-shrink-0 ${selectedFilter === filter ? 'bg-[#3d2b1f] text-[#ac9181] shadow-sm font-bold' : 'bg-[#f3ecea] text-[#4f453f] border border-[#d2c4bc]'}`}>{label}</button>)}
        </div>
      </section>

      <div className="space-y-4">
        {filteredOrders.length === 0 ? <div className="text-center py-12 bg-[#f9f2f0] rounded-2xl border border-[#f3ecea] p-6"><div className="w-12 h-12 bg-[#e1e1c9] rounded-full flex items-center justify-center mx-auto mb-3 text-[#636451]"><span className="material-symbols-outlined">coffee</span></div><h4 className="font-serif text-lg font-bold text-[#26170c]">No orders in this state</h4><p className="text-xs text-[#4f453f] mt-1">{selectedFilter === 'Completed' ? 'Completed orders will appear here as they are picked up.' : 'All caught up! Tap the + button to enter a new counter order.'}</p></div> : filteredOrders.map((order, idx) => {
          const isNew = order.status === 'New'; const isBrewing = order.status === 'Brewing' || order.status === 'Preparing'; const isReady = order.status === 'Ready'; const isCompleted = order.status === 'Completed';
          const borderClass = isNew ? 'border-l-4 border-[#ba1a1a]' : isReady ? 'border-l-4 border-[#26170c]' : isCompleted ? 'border-l-4 border-[#81756e]' : 'border-l-4 border-[#5e604d]';
          const badgeClass = isNew ? 'bg-[#ffdad6] text-[#93000a]' : isReady ? 'bg-[#3d2b1f] text-[#ac9181]' : isCompleted ? 'bg-[#e8e1df] text-[#4f453f]' : 'bg-[#e1e1c9] text-[#636451]';
          const statusLabel = isNew ? 'NEW' : isReady ? 'READY' : isCompleted ? 'COMPLETED' : 'BREWING';
          return <React.Fragment key={order.id}>
            <article className={`bg-[#f9f2f0] rounded-2xl p-3.5 sm:p-4 shadow-[0_4px_12px_rgba(61,43,31,0.06)] ${borderClass} flex flex-col gap-3 transition-all relative border border-[#f3ecea]`}>
              ...
              <div className="flex items-center gap-2 pt-1">
                {isNew && <><button onClick={() => handleStatusAdvance(order)} className="flex-1 bg-[#8FBC8F] hover:bg-[#7ea97e] text-[#26170c] font-bold text-sm py-2.5 rounded-full shadow-sm">Accept Order</button><div className="relative"><button onClick={() => setActiveMenuOrderId(activeMenuOrderId === order.id ? null : order.id)} className="p-2 text-[#4f453f] rounded-full border border-[#d2c4bc] flex items-center justify-center"><span className="material-symbols-outlined text-[20px]">more_vert</span></button>{activeMenuOrderId === order.id && <div className="absolute right-0 bottom-full mb-2 w-44 bg-white rounded-xl shadow-xl border border-[#e8e1df] py-1 z-20 text-xs"><button onClick={() => { onShowNotification(`Printed Barista ticket #${order.orderNumber}`); setActiveMenuOrderId(null); }} className="w-full text-left px-3 py-2 hover:bg-[#f3ecea] text-[#26170c] flex items-center gap-2"><span className="material-symbols-outlined text-[16px]">print</span>Print Barista Chit</button><button onClick={() => { handleRemindCustomer(order); setActiveMenuOrderId(null); }} className="w-full text-left px-3 py-2 hover:bg-[#f3ecea] text-[#26170c] flex items-center gap-2"><span className="material-symbols-outlined text-[16px]">sms</span>Text Customer</button></div>}</div></>}
                {isBrewing && <button onClick={() => handleStatusAdvance(order)} className="flex-1 bg-[#26170c] hover:bg-[#3d2b1f] text-white font-semibold text-sm py-2.5 rounded-full shadow-sm flex items-center justify-center gap-1.5"><span className="material-symbols-outlined text-[18px]">check_circle</span>Ready for Pickup</button>}
                {isReady && <><button onClick={() => handleStatusAdvance(order)} className="flex-1 bg-[#26170c] hover:bg-[#3d2b1f] text-white font-semibold text-sm py-2.5 rounded-full shadow-sm flex items-center justify-center gap-1.5"><span className="material-symbols-outlined text-[18px]">done_all</span>Complete Order</button><button onClick={() => handleRemindCustomer(order)} className="bg-[#f3ecea] text-[#4f453f] px-4 py-2.5 rounded-full text-xs font-semibold border border-[#d2c4bc]">Remind</button></>}
      ...
      {paymentOrder && <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs" role="dialog" aria-modal="true" aria-labelledby="payment-method-title">
        <div className="w-full max-w-md bg-[#fff8f5] rounded-2xl shadow-2xl border border-[#e8e1df] overflow-hidden">
          ...
          <div className="p-5">
            ...
            {paymentMode === 'single' ? <div className="grid grid-cols-2 gap-3">{PAYMENT_METHODS.map(method => <button key={method} type="button" onClick={() => { setSinglePayment(method); setPaymentError(''); }} className={`p-3 rounded-xl border font-bold ${singlePayment === method ? 'bg-[#26170c] text-white border-[#26170c]' : 'bg-white text-[#4f453f] border-[#d2c4bc]'}`}>{method}</button>)}</div> : <div className="space-y-3">
              <p className="text-xs text-[#81756e]">Combine two or more payment methods. The amounts must total exactly {money(paymentOrder.total)}.</p>
              {splitLines.map((line, index) => <div key={index} className="grid grid-cols-[1fr_110px_36px] gap-2 items-center"><select value={line.method} onChange={e => { const next = [...splitLines]; next[index] = { ...next[index], method: e.target.value as PaymentMethod }; setSplitLines(next); setPaymentError(''); }} className="w-full p-2.5 rounded-lg border border-[#d2c4bc] bg-white text-sm font-semibold">{PAYMENT_METHODS.map(method => <option key={method} value={method}>{method}</option>)}</select><input type="number" min="0" step="0.01" value={line.amount || ''} onChange={e => { const next = [...splitLines]; next[index] = { ...next[index], amount: Number(e.target.value) || 0 }; setSplitLines(next); setPaymentError(''); }} placeholder="Amount" className="w-full p-2.5 rounded-lg border border-[#d2c4bc] bg-white text-sm text-right" />{splitLines.length > 2 ? <button type="button" onClick={() => setSplitLines(splitLines.filter((_, i) => i !== index))} className="w-9 h-9 rounded-lg bg-[#eee2dd] text-[#4f453f] font-bold">×</button> : <span />}</div>)}
              <button type="button" onClick={() => setSplitLines([...splitLines, { method: 'Cash', amount: 0 }])} className="w-full py-2.5 rounded-xl border border-dashed border-[#5e604d] bg-white text-[#5e604d] text-sm font-bold">+ Add Payment Method</button>
              <div className={`rounded-xl p-3 text-sm font-bold flex justify-between ${splitMatches ? 'bg-[#e1e1c9] text-[#5e604d]' : 'bg-[#ffdad6] text-[#93000a]'}`}><span>Entered: {money(splitTotal)}</span><span>{splitMatches ? '✓ Matches Total' : `Remaining: ${money(Math.max(0, orderTotal - splitTotal))}`}</span></div>
            </div>}
            {paymentError && <p className="text-xs text-[#a52b22] mt-3">{paymentError}</p>}
            <button type="button" onClick={completeWithPayment} className="w-full mt-4 bg-[#26170c] hover:bg-[#3d2b1f] text-white font-bold py-3 rounded-xl">Complete Order</button>
          </div>
        </div>
      </div>}
    </div>
