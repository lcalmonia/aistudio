...
export const SettingsView: React.FC<SettingsViewProps> = ({ settings, onSaveSettings, onResetSettings, onShowNotification, onSwitchToCustomerPortal }) => {
  ...
  useEffect(() => {
    deliveryZoneService.list().then((zones) => setDeliveryZones(zones)).catch((error) => console.warn('[SettingsView] Delivery zones could not be loaded:', error)).finally(() => setZonesLoaded(true));
  }, []);

  const handleInputChange = (field: keyof StoreSettings, value: any) => setFormData((prev) => ({ ...prev, [field]: value }));
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      onShowNotification(error instanceof Error ? error.message : 'The logo could not be prepared.');
    }
  };
  ...
  const updateZone = (index: number, patch: Partial<DeliveryZone>) => setDeliveryZones((prev) => prev.map((zone, i) => i === index ? { ...zone, ...patch } : zone));

  const addZone = () => setDeliveryZones((prev) => [...prev, {
    id: `zone-${Date.now()}`,
  ...
  const removeZone = (index: number) => setDeliveryZones((prev) => prev.filter((_, i) => i !== index).map((zone, i) => ({ ...zone, priority: i + 1 })));
    ...
    if (window.confirm('Reset all store settings to original defaults?')) {
      if (onResetSettings) {
        try {
          ...
          if (resetRes) { setFormData({ ...resetRes }); setBaseSettings({ ...resetRes }); }
        } catch (err) { console.error('[SettingsView] Reset error:', err); }
      } else setFormData({ ...baseSettings });
    }
  };
    ...
    <div className="pt-20 px-3 sm:px-5 max-w-3xl mx-auto pb-28">
      <div className="mb-3">
        <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-[#dec1af]/40">
          <div><div className="flex items-center gap-1.5"><span className="px-1.5 py-0.5 bg-[#26170c] text-white text-[9px] font-bold rounded">ADMIN</span><h2 className="font-serif text-lg sm:text-xl font-bold text-[#26170c] tracking-tight leading-tight">Store & Branding Settings</h2></div><p className="text-[11px] text-[#6e635d] mt-0.5">Manage store name, logo, branch details, and receipt info.</p></div>
