...
import { AdminPrincipal, CustomerUser, Order } from '../../types';
...
export const CustomerManagementView: React.FC<CustomerManagementViewProps> = ({
  ...
  useEffect(() => {
    ...
    customerService
      ...
      .then((serverCustomers) => {
        ...
        const localOnly = (customers || []).filter(
          (c) => !serverIds.has(c.id) && !serverEmails.has((c.email || '').toLowerCase())
        );
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customerList.filter((c) => c && (
  ...
  const refreshDirectory = async () => {
    try {
      const serverCustomers = await customerService.listCustomersFromServer();
      setDirectoryCustomers(serverCustomers);
    } catch (error) {
      console.warn('[CustomerManagementView] Failed to refresh directory:', error);
    }
  };
  ...
  const handleApproveReset = async (requestId: string) => {
    ...
    try {
      await customerService.approvePasswordReset(requestId);
      onShowNotification('Password reset approved. Customer password is now password1234.');
      await refreshResetRequests();
    } catch (error) {
      onShowNotification(`Unable to approve password reset: ${error instanceof Error ? error.message : 'Server error'}`);
    } finally {
  ...
  const handleRejectReset = async (requestId: string) => {
    setRequestLoading(true);
    try {
      await customerService.rejectPasswordReset(requestId);
      onShowNotification('Password reset request rejected.');
      await refreshResetRequests();
    } catch (error) {
      onShowNotification(`Unable to reject password reset: ${error instanceof Error ? error.message : 'Server error'}`);
    } finally {
      setRequestLoading(false);
    }
  };
  ...
  const handleDirectReset = async (customerId: string) => {
    ...
    setRequestLoading(true);
    try {
      await customerService.resetCustomerPassword(customerId);
      onShowNotification('Customer password reset to password1234.');
    } catch (error) {
      onShowNotification(`Unable to reset password: ${error instanceof Error ? error.message : 'Server error'}`);
    } finally {
      setRequestLoading(false);
    }
  };
  ...
  const handleDeleteCustomer = async (customer: CustomerUser) => {
    ...
    try {
      ...
      onShowNotification(`Customer ${customer.name} was deleted.`);
    } catch (error) {
      onShowNotification(`Unable to delete customer: ${error instanceof Error ? error.message : 'Server error'}`);
    } finally {
      setRequestLoading(false);
    }
  };
  ...
  return (
    <div className="pt-20 px-3.5 sm:px-5 max-w-5xl mx-auto pb-28 space-y-5">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          <h2 className="font-serif text-2xl sm:text-[28px] font-bold text-[#26170c] tracking-tight">Customer Directory</h2>
          <p className="text-xs text-[#4f453f] mt-0.5">Registered customer accounts, contact details & lifetime ordering history</p>
        </div>
      ...
      {isSuperAdmin && resetRequests.length > 0 && (
        <div className="bg-[#fff3e8] border border-[#e0a66d] rounded-2xl p-4 space-y-3 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[#9a4b00]">notifications_active</span>
              <div>
                <h3 className="text-sm font-bold text-[#26170c]">Password Reset Requests ({resetRequests.length})</h3>
                <p className="text-[11px] text-[#6d5543]">Super Admin approval is required before a customer password changes.</p>
              </div>
            </div>
          </div>
          {resetRequests.map((request) => (
            <div key={request.id} className="bg-white rounded-xl border border-[#ead8ca] p-3 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
              <div className="min-w-0 text-xs">
                <div className="font-bold text-[#26170c]">{request.customerName} <span className="font-mono text-[10px]">({request.customerId})</span></div>
                <div className="text-[#6d5543] truncate">{request.email} • {request.mobile}</div>
                <div className="text-[10px] text-[#81756e] mt-1">Requested {new Date(request.requestedAt).toLocaleString()}</div>
              </div>
              <div className="flex flex-wrap gap-2 flex-shrink-0">
                <button type="button" disabled={requestLoading} onClick={() => void handleApproveReset(request.id)} className="px-3 py-2 rounded-lg bg-[#26170c] text-white text-[11px] font-bold disabled:opacity-50 cursor-pointer">Approve & Reset</button>
                <button type="button" disabled={requestLoading} onClick={() => void handleRejectReset(request.id)} className="px-3 py-2 rounded-lg bg-white border border-[#dec1af] text-[#26170c] text-[11px] font-bold disabled:opacity-50 cursor-pointer">Reject</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="relative">
        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by Customer ID, name, email, or phone number..." className="w-full pl-10 pr-4 py-3 bg-white border border-[#dec1af] rounded-2xl text-xs sm:text-sm text-[#26170c] shadow-xs focus:outline-none focus:ring-2 focus:ring-[#26170c]" />
        <span className="material-symbols-outlined absolute left-3 top-3 text-[#81756e] text-[20px]">search</span>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <div className={`space-y-3 ${selectedCustomer ? 'lg:col-span-6' : 'lg:col-span-12'}`}>
          {filteredCustomers.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-2xl border border-[#f3ecea]"><span className="material-symbols-outlined text-[32px] text-[#81756e] mb-2">person_off</span><p className="text-sm font-bold text-[#26170c]">No registered customers found</p><p className="text-xs text-[#4f453f] mt-1">Try adjusting your search terms</p></div>
          ) : filteredCustomers.map((cust) => {
            const custOrders = getCustomerOrders(cust.id);
            const totalSpent = getCustomerTotalSpend(cust.id);
            const isSelected = cust.id === selectedCustomerId;
            return (
              <div key={cust.id} onClick={() => setSelectedCustomerId(isSelected ? null : cust.id)} className={`p-4 bg-white rounded-2xl border transition-all cursor-pointer shadow-xs hover:shadow-md ${isSelected ? 'border-[#26170c] ring-2 ring-[#26170c]/20 bg-[#fff8f5]' : 'border-[#f3ecea] hover:border-[#dec1af]'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0"><div className="w-10 h-10 rounded-full bg-[#26170c] text-white flex items-center justify-center font-bold text-sm flex-shrink-0">{cust.name.charAt(0)}</div><div className="min-w-0"><div className="flex items-center gap-1.5 flex-wrap"><h4 className="font-serif text-sm sm:text-base font-bold text-[#26170c]">{cust.name}</h4><span className="px-2 py-0.2 bg-[#e1e1c9] text-[#636451] font-mono text-[10px] font-bold rounded-md">{cust.id}</span></div><p className="text-xs text-[#4f453f] mt-0.5 truncate">{cust.email} • {cust.mobile}</p></div></div>
                  <span className="px-2 py-0.5 bg-[#8fbc8f]/30 text-[#26170c] text-[10px] font-bold rounded-full flex-shrink-0">{cust.status.toUpperCase()}</span>
                </div>
