...
export const CustomerManagementView: React.FC<CustomerManagementViewProps> = ({
  ...
  useEffect(() => {
    if (!directoryCustomers.length && customers.length) {
      setDirectoryCustomers(customers);
    }
              ...
              return (
                <div
                  ...
                  className={`p-4 bg-white rounded-2xl border transition-all cursor-pointer shadow-xs hover:shadow-md ${
                    isSelected ? 'border-[#26170c] ring-2 ring-[#26170c]/20 bg-[#fff8f5]' : 'border-[#f3ecea] hover:border-[#dec1af]'
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-full bg-[#26170c] text-white flex items-center justify-center font-bold text-sm flex-shrink-0">{cust.name.charAt(0)}</div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <h4 className="font-serif text-sm sm:text-base font-bold text-[#26170c]">{cust.name}</h4>
                          <span className="px-2 py-0.2 bg-[#e1e1c9] text-[#636451] font-mono text-[10px] font-bold rounded-md">{cust.id}</span>
                        </div>
                        <p className="text-xs text-[#4f453f] mt-0.5 truncate">{cust.email} • {cust.mobile}</p>
                      </div>
                    </div>
                  ...
                  <div className="mt-3 pt-2.5 border-t border-[#f3ecea] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs text-[#4f453f]">
                    ...
                    <span className="text-[#81756e] text-[11px] font-medium">Joined {cust.createdAt}</span>
                  </div>
                </div>
              );
            })
        ...
        {selectedCustomer && (
          <div className="lg:col-span-6 bg-white rounded-3xl border border-[#dec1af] p-5 shadow-lg space-y-4">
            <div className="flex justify-between items-start border-b border-[#f3ecea] pb-3">
              <div>
                <span className="text-[10px] font-mono font-bold px-2 py-0.5 bg-[#26170c] text-[#fbddca] rounded-md">{selectedCustomer.id}</span>
                <h3 className="font-serif text-xl font-bold text-[#26170c] mt-1">{selectedCustomer.name}</h3>
                <p className="text-xs text-[#81756e]">Member since {selectedCustomer.createdAt}</p>
              </div>
              <button type="button" onClick={() => setSelectedCustomerId(null)} className="p-1 text-[#81756e] hover:text-[#26170c] rounded-lg cursor-pointer">
                <span className="material-symbols-outlined text-[20px]">close</span>
              </button>
            </div>

            <div className="bg-[#f9f2f0] p-3.5 rounded-2xl space-y-2 text-xs">
              <div><span className="text-[#81756e] block text-[10px] uppercase font-bold">Email Address</span><span className="font-semibold text-[#26170c]">{selectedCustomer.email}</span></div>
              <div><span className="text-[#81756e] block text-[10px] uppercase font-bold">Mobile Phone</span><span className="font-semibold text-[#26170c]">{selectedCustomer.mobile}</span></div>
              <div><span className="text-[#81756e] block text-[10px] uppercase font-bold">Saved Delivery Address</span><span className="font-semibold text-[#26170c]">{selectedCustomer.address}</span></div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-[#e1e1c9]/40 rounded-2xl border border-[#dec1af]/30 text-center">
                ...
                <span className="font-serif text-xl font-extrabold text-[#26170c]">{selectedCustomer.stamps || 0} / 10</span>
              ...
              <div className="p-3 bg-[#fbddca]/40 rounded-2xl border border-[#dec1af]/30 text-center">
                ...
                <span className="font-serif text-xl font-extrabold text-[#26170c]">{selectedCustomer.points || 0} pts</span>
                <span className="text-[10px] text-[#81756e] block mt-0.5">VIP Tier Member</span>
              </div>
            </div>

            <div>
              <h4 className="font-serif text-sm font-bold text-[#26170c] mb-2 flex items-center justify-between gap-2">
                <span>Order History ({selectedCustomerOrders.length})</span>
                <span className="text-xs font-normal text-[#81756e]">Total Spend: ₱{getCustomerTotalSpend(selectedCustomer.id).toFixed(2)}</span>
              </h4>

              {selectedCustomerOrders.length === 0 ? (
                <p className="text-xs text-[#81756e] bg-[#f9f2f0] p-4 rounded-xl text-center">No orders placed yet.</p>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {selectedCustomerOrders.map((ord) => (
                    <div key={ord.id} className="p-3 bg-[#f9f2f0] rounded-xl border border-[#f3ecea] text-xs flex justify-between items-center gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-bold text-[#26170c]">#{ord.orderNumber}</span>
                          <span className="px-1.5 py-0.2 bg-[#dec1af]/50 text-[#26170c] rounded text-[10px] font-bold">{ord.status}</span>
                          <span className="text-[10px] text-[#81756e]">• {ord.orderType || 'Order'}</span>
                        </div>
                        <p className="text-[#4f453f] text-[11px] mt-0.5 truncate">{ord.items.map((i) => `${i.quantity}x ${i.name}`).join(', ')}</p>
                      </div>
                      <span className="font-bold text-sm text-[#26170c] flex-shrink-0">₱{ord.total.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              )}
