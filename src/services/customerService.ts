...
export const customerService = {
  ...
  async createCustomer(data:{name:string;username:string;email:string;mobile:string;address:string;password?:string}):Promise<{success:boolean;customer?:CustomerUser;error?:string}>{
    const customers=storageAdapter.getCustomers(); const cleanUsername=data.username.trim().toLowerCase(); const cleanEmail=data.email.trim().toLowerCase(); const cleanMobile=data.mobile.trim();
    ...
    if(customers.find(c=>(c.username || '').toLowerCase()===cleanUsername)) return {success:false,error:'That username is already in use. Please choose another.'};
    const newCustomer:CustomerUser={id:generateCustomerId(),name:data.name.trim(),username:cleanUsername,email:cleanEmail,mobile:cleanMobile,address:data.address.trim(),createdAt:new Date().toISOString().split('T')[0],status:'active',role:'customer',stamps:0,points:0};
