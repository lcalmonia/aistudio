...
export const loyaltyService = {
  async getCustomerLoyalty(customerId: string): Promise<{ stamps:number; points:number } | null> {
    ...
    return {stamps:customer.stamps||0,points:customer.points||0};
  },
  async addPoints(customerId:string,pointsEarned:number,referenceOrderId?:string,reason:string='Order loyalty reward'):Promise<number|null>{
    const customer=await customerService.getCustomer(customerId);if(!customer)return null;
    const newPoints=(customer.points||0)+Math.max(0,Math.floor(pointsEarned));
    await customerService.updateCustomer(customerId,{points:newPoints});
    if(pointsEarned>0)storageAdapter.addLoyaltyTransaction({id:generateEntityId('loy'),customerId,type:'earn_points',amount:Math.floor(pointsEarned),referenceOrderId,reason,timestamp:Date.now(),createdAt:new Date().toISOString()});
    return newPoints;
  },
  async addStamp(customerId:string,maxStamps:number=10,referenceOrderId?:string,reason:string='Order loyalty stamp'):Promise<{stamps:number;unlockedReward:boolean}|null>{
    const customer=await customerService.getCustomer(customerId);if(!customer)return null;
    const currentStamps=customer.stamps||0;const nextStamps=(currentStamps%maxStamps)+1;const unlockedReward=nextStamps===maxStamps;
    await customerService.updateCustomer(customerId,{stamps:nextStamps});
    storageAdapter.addLoyaltyTransaction({id:generateEntityId('loy'),customerId,type:'earn_stamps',amount:1,referenceOrderId,reason:unlockedReward?`${reason} (Reward Unlocked!)`:reason,timestamp:Date.now(),createdAt:new Date().toISOString()});
  ...
  async awardOrderRewards(customerId:string,order:Order):Promise<{stampsAdded:number;pointsAdded:number;qualified:boolean;reason:string}> {
    // Rewards are earned only after the order is actually marked Completed.
    if(order.status!=='Completed')return {stampsAdded:0,pointsAdded:0,qualified:false,reason:'Order is not Completed yet; no loyalty reward awarded.'};
    ...
    if(orderType==='Delivery') {
      ...
      if(points>0)await this.addPoints(customerId,points,order.id,'Completed delivery order reward points');
    ...
    if(orderType==='Dine-In'||orderType==='Takeout') {
      ...
      const count=Math.max(1,Math.floor(s.stampsPerQualifyingOrder));let added=0;
      for(let i=0;i<count;i++){await this.addStamp(customerId,s.stampCycle,order.id,'Completed qualifying Dine-In/Takeout order');added++;}
