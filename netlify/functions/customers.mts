...
export default async function handler(request: Request): Promise<Response> {
  try {
    ...
    if (request.method === 'POST') {
      ...
      const db=database(); const existing=await db.pool.query(`SELECT id FROM customers WHERE email=$1 OR id=$2 OR LOWER(username)=$3 LIMIT 1`,[email,id,username]); if(existing.rows.length>0)throw new RequestError(409,'A customer account with this email or ID already exists.');
