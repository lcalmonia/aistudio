...
export default async function handler(request: Request, context: Context): Promise<Response> {
  ...
  try {
    ...
    if (!canResetAdminAccountPassword(actor, target.id, target.created_by_staff_user_id)) {
      throw new RequestError(403, 'You are not authorized to reset this Admin account password.');
    }

    const body = await readJsonObject(request);
    const passwordHash = await hashPassword(validatePassword(body.password));
    await changeAdminPassword(accountId, passwordHash);

    if (isSelf) clearAdminSessionCookie(context);
    return json({ changed: true, reauthenticationRequired: isSelf });
  } catch (error) {
...
export const config: Config = {
  path: '/api/admin/accounts/:id/password',
  method: 'POST',
};
